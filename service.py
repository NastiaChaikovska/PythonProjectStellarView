...
def run_program(image_name):
    ...
    print('\n\ntriples_of_stars: ', triples_of_stars, '\n\n')
    ...
    return f"Your  location  is:  {round(avg_latitude, 4)},  {round(avg_longitude, 4)}"
    # return f"Your  location  is:  {avg_latitude},  {avg_longitude}"


def find_nearest_star_by_coordinates(ra, dec):
    query = {
        "location": SON([("$near", {"$geometry": {"type": "Point", "coordinates": [ra, dec]}})])
    }
    result = collection.find_one(query)
    return result


def find_max_mag_star():
    sort_query = [("mag", pymongo.DESCENDING)]   # max
    max_mag = collection.find_one({}, sort=sort_query)['mag']
    return max_mag
...
def get_stars_to_explore(latitude, longitude):
    calculate_new_mag()
    central_star = find_nearest_star_by_coordinates(latitude, longitude)
    central_star_ra = central_star['location']['coordinates'][0]
    central_star_dec = central_star['location']['coordinates'][1]
    result_stars = []
    for curr_star in collection.find():
        result_stars.append([
